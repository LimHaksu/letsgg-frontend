...
interface Props {
    dialogOpenList?: boolean[];
    setDialogOpenList?: (idx : number, flag: boolean) => {};
}

function StoreRegisterDialog({ dialogOpenList, setDialogOpenList }: Props) {
    ...
    const handleClickOpen = () => {
        setDialogOpenList!(0, true);
    };
    const handleClose = () => {
        setDialogOpenList!(0, false);
        clearInputs();
    };
    ...
    return (
        <div>
            <Dialog fullScreen onClose={handleClose} aria-labelledby="customized-dialog-title" open={dialogOpenList![0]}>
...
export default inject(({ window }) => ({
    dialogOpenList: window.dialogOpenList,
